...
# Create a program that goes through and counts the occurrence of each character, 
# excluding \n using a  dictionary
...
# Output: each letter and its corresponding occurrence in alphabetical order
total_char=sum(char_count.values())
sort_count=dict(sorted(char_count.items()))
print("*"*75)
# Output which letter occurred the most 
...
# Output what the percentage of the string each character is, again in alphabetical
print("Charcter occurences and percentages in alphavetical order:")
for char, count in sort_count.items():
    percentage= (count/total_char)*100
    print(f"{char}: {count} occurrences which is {percentage}% of the string")